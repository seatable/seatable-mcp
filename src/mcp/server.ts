        ...
        this.server = new Server(
            ...
            },
            {
                ...
                instructions: [
                    ...
                    'Prefer query_sql for reading, updating, and deleting data. It supports SELECT, UPDATE...WHERE, and DELETE...WHERE in a single call — much more efficient than fetching row IDs first. SELECT returns max 10,000 rows (vs. list_rows max 1,000 per page).',
                    '',
                    'SeaTable SQL does NOT use MySQL/MariaDB function names. You MUST use SeaTable function names:',
                    ...
                    '- REPEAT → rept(str, n)',
                    '- LOCATE/INSTR → find(substr, str) (case-sensitive) or search(substr, str) (case-insensitive)',
                    '- UCASE → upper(str), LCASE → lower(str)',
                    '- FORMAT → text(num, format)  (formats: "number", "percent", "dollar", "euro", "yuan")',
                    '- CURDATE/CURRENT_DATE → today(), DATE_ADD → dateAdd(date, n, "unit"), DATE_SUB → dateAdd(date, -n, "unit")',
                    '- DATEDIFF/TIMESTAMPDIFF → dateDif(d1, d2, unit)  (unit: "D", "M", "Y", "S", "YD", "YM", "MD")',
                    '- EXTRACT(YEAR/MONTH/DAY FROM date) → year(date), month(date), day(date)',
                    '- DAYOFWEEK → weekday(date, weekStart), WEEK → weeknum(date, return_type)  (use 21 for ISO weeks)',
                    '- LAST_DAY → eomonth(date, 0)',
                    '- CEIL → ceiling(n), POW → power(a, b), LOG10 → lg(n), TRUNCATE → rounddown(n, d)',
                    '- CASE WHEN...THEN...END → if(cond, trueVal, falseVal) or ifs(cond1, val1, cond2, val2, ...)',
                    '- NOT supported: CAST, CONVERT, GREATEST, LEAST, COALESCE, IFNULL, NULLIF, RAND, GROUP_CONCAT, LPAD, RPAD, REVERSE, DAYNAME, MONTHNAME, STR_TO_DATE, LOG2 (use log(n, 2))',
                    '',
                    'Functions can be used in SELECT, WHERE, GROUP BY, HAVING, ORDER BY. They are NOT supported in UPDATE SET or INSERT VALUES — only constant values there.',
